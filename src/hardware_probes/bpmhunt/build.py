#!/usr/bin/env python3
"""Build BpmHunt.ZDL — the memory-inspector probe."""
...
def main() -> None:
    ...
    write_param_header(manifest, HERE / "bpmhunt_params.h", "BPMHUNT")

    src_c = HERE / "bpmhunt.c"
    obj = HERE / "bpmhunt.obj"
    out_zdl = ROOT / "dist" / f"{manifest['effect_name']}.ZDL"
    out_zdl.parent.mkdir(exist_ok=True)

    print(f"[bpmhunt] compiling {src_c.name} -> {obj.name}")
    subprocess.run(
    ...
    cfg = LinkerConfig(
        ...
        screen_image=make_airwindows_tape_screen("Bpm", "Hunt"),
        # UNPATCHED LineSel handlers — state[31] is the safe table lookup.
        # The audio function does all the work.
        audio_nop=manifest.get("audio_nop", False),
    )
    link(cfg)

    print(f"\n[bpmhunt] done -> {out_zdl}")
